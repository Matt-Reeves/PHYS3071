...
int main(){
    ...
    for (int j =0; j<N; j++){
        for (int k =0; k<N; k++){        
            //phi[j][k] = 0.5*sin(M_PI*x[j]) +0.5*sin(M_PI*x[k]);
            //phi2[j][k] = 0.5*sin(M_PI*x[j])+ 0.5*sin(M_PI*x[k]);
            phi[j][k] = 0.0;
            phi2[j][k] = 0.0;
    ...
    for (int j = 0; j<N; j++){
        for (int k = 0; k<N; k++){
            ...
            if (z%2 == 0)
             {
                ...
                {
                    phi[j][k] = 0.25*( phi2[j+1][k] + phi2[j-1][k] 
                             + phi2[j][k+1] + phi2[j][k-1] 
                             + dx*dx*f[j][k] );
                ...
                if (j ==0 || k == 0 || j == N-1 || k == N-1 )
            {
                ...
                phi2[j][k] = 0.25*( phi[j+1][k] + phi[j-1][k] 
                             + phi[j][k+1] + phi[j][k-1] 
                             + dx*dx*f[j][k] );
            }
                SSE[z] += pow(phi2[j][k] - phi_exact[j][k],2.0);
            }
        }
    }
}
...
std::ofstream saveFile, errFile;
  ...
  saveFile.open("Jacobi.txt");
  
  errFile.precision(16);
  errFile.open("JacobiErr.txt");
